...
class Node implements Comparable<Node> {
  ...
  @override
  String toString() => '($x, $y) -> Costo: $cost, Nivel: $level, Orden: $orderInLevel';
}
  ...
  Future<Node?> search(Future<void> Function(Node, [bool]) renderNode) async {
    ...
    while (queue.isNotEmpty) {
      ...
      for (var advance in advanceOrders) {
        ...
        if (current.father != null && newX == current.father!.x && newY == current.father!.y) {
          continue;  
        }
        if (isValid(newX, newY, board)) {
          int heuristic = manhattanDistance(newX, newY, goalX, goalY);
          int level = current.level + 1;
          int orderInLevel = levelOrderCounter.update(level, (value) => value + 1, ifAbsent: () => 0);
          Node neighbor = Node(newX, newY, heuristic, level, orderInLevel, current);
          queue.add(neighbor);
